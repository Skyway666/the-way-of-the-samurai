...
bool string_comparator(char* str, char*str2){
	int counter = 0;
	int counter2 = 0;
...
int input_number(int max_number)
{
	while (true)
	{
		...
		char orden[100];
		scanf_s("%s", orden, 100);
		std::string str = orden;
		int input_number = std::atoi(orden);
...
void movement(position * position, char* orden) {
	...
	if (string_comparator(orden, "arriba"))
		position->x--;
	if (string_comparator(orden, "abajo"))
		position->x++;
	if (string_comparator(orden, "derecha"))
		position->y++;
	if (string_comparator(orden, "izquierda"))
		position->y--;
...
void tutorial() {
	...
	while (true){
		...
		if (string_comparator(orden, movement_objective))
			tutorial_step++;
		else{
...
void orden_com(char *orden){
	...
	while (!valid_command){
		...
		if (!string_comparator(orden, "arriba") && !string_comparator(orden, "abajo") && !string_comparator(orden, "derecha") && !string_comparator(orden, "izquierda") && !string_comparator(orden, "inventario"))
